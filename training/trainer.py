...
class DiceBCELoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        dice_loss = 1.0 - (2 * intersection + eps) / (pred_prob.sum(dim=(2, 3)) + target.sum(dim=(2, 3)) + eps)
        dice_loss = dice_loss.mean()
        return self.dice_w * dice_loss + self.bce_w * bce_loss

class Trainer:

    def __init__(self, model, train_loader, val_loader, device, lr=0.0001, weight_decay=1e-05, checkpoint_dir='results/checkpoints', log_dir='results/logs', experiment_name='finetune'):
        ...
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=50, eta_min=1e-06)
    ...
    def validate(self):
        ...
        return {'val_loss': total_loss / n, 'val_dice': sum(all_dice) / len(all_dice), 'val_iou': sum(all_iou) / len(all_iou), 'val_pixel_acc': sum(all_acc) / len(all_acc)}
    ...
    def fit(self, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            self.logger.info(f"Epoch {epoch:3d}/{epochs} | loss={train_loss:.4f} | val_loss={val_metrics['val_loss']:.4f} | dice={val_metrics['val_dice']:.4f} | iou={val_metrics['val_iou']:.4f} | lr={self.optimizer.param_groups[0]['lr']:.2e} | {elapsed:.1f}s")
        ...
        save_json(self.history, os.path.join(self.log_dir, f'{self.experiment_name}_history.json'))
        self.logger.info(f'Training complete. Best Dice: {self.best_dice:.4f}')
        return self.history

    def save_checkpoint(self, filename, epoch, metrics):
        path = os.path.join(self.checkpoint_dir, filename)
        torch.save({'epoch': epoch, 'model_state_dict': self.model.state_dict(), 'optimizer_state_dict': self.optimizer.state_dict(), 'metrics': metrics, 'best_dice': self.best_dice}, path)
