...
def dice_coefficient(pred, target, threshold=0.5, eps=1e-06):
...
def iou_score(pred, target, threshold=0.5, eps=1e-06):
    pred = torch.sigmoid(pred) if pred.min() < 0 or pred.max() > 1 else pred
    pred_bin = (pred > threshold).float()
    intersection = (pred_bin * target).sum(dim=(1, 2, 3))
...
def compute_all_metrics(pred, target, threshold=0.5):
    return {'dice': dice_coefficient(pred, target, threshold), 'iou': iou_score(pred, target, threshold), 'pixel_accuracy': pixel_accuracy(pred, target, threshold)}
