import os
import numpy as np
from src.inference.onnx_infer import create_ort_session, export_to_onnx, run_onnx_benchmark
...
def _import_quant_tools():
    try:
        ...
        raise RuntimeError('onnxruntime quantization tools are required for PTQ.') from exc
    return {'CalibrationMethod': CalibrationMethod, 'QuantFormat': QuantFormat, 'QuantType': QuantType, 'quantize_dynamic': quantize_dynamic, 'quantize_static': quantize_static}
...
def run_ptq_benchmark(model, calibration_loader, test_loader, device, output_dir='results', input_size=(256, 256), warmup=20, latency_runs=100, batch_size_throughput=16, calibration_batches=32, force_dynamic=False):
    ...
    reader = _LoaderDataReader(calibration_loader, input_name=input_name, max_batches=calibration_batches, input_dtype=np.float32)
    quant_mode = 'static'
    fallback_reason = None
    if force_dynamic:
        quant_mode = 'dynamic'
        logger.info('Running forced dynamic quantization...')
        tools['quantize_dynamic'](fp32_onnx, int8_onnx, weight_type=tools['QuantType'].QInt8)
    else:
        try:
            logger.info('Running static PTQ quantization (QDQ + per-channel)...')
            tools['quantize_static'](fp32_onnx, int8_onnx, reader, quant_format=tools['QuantFormat'].QDQ, activation_type=tools['QuantType'].QUInt8, weight_type=tools['QuantType'].QInt8, per_channel=True, calibrate_method=tools['CalibrationMethod'].MinMax)
        except Exception as exc:
            logger.warning('Static PTQ failed (%s). Falling back to dynamic PTQ.', exc)
            quant_mode = 'dynamic'
            fallback_reason = str(exc)
            tools['quantize_dynamic'](fp32_onnx, int8_onnx, weight_type=tools['QuantType'].QInt8)
    logger.info('Running benchmark on PTQ model -> %s', int8_onnx)
    try:
        result = run_onnx_benchmark(model=None, test_loader=test_loader, device=device, output_dir=output_dir, precision='int8', warmup=warmup, latency_runs=latency_runs, batch_size_throughput=batch_size_throughput, input_size=input_size, onnx_path=int8_onnx, method_name='PTQ (int8)', result_filename='ptq_result.json', export_if_missing=False)
    except Exception as exc:
        logger.warning('Skipping PTQ runtime benchmark: %s', exc)
        result = {'method': 'PTQ (int8)', 'precision': 'int8', 'device': device, 'runtime_device': device, 'status': 'skipped', 'error': str(exc), 'onnx_path': int8_onnx}
