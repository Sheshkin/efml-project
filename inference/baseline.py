...
def evaluate_model(model, data_loader, device, precision='fp32'):
    ...
    with torch.no_grad():
        for (imgs, masks) in tqdm(data_loader, desc='Evaluating', leave=False):
            ...
            if precision == 'fp16':
                imgs = imgs.half()
                with torch.autocast(device_type=device if device != 'mps' else 'cpu', dtype=torch.float16, enabled=device != 'mps'):
                    preds = model(imgs)
            else:
                preds = model(imgs)
            preds = preds.float()
...
def run_baseline_benchmark(model, test_loader, device, precision='fp16', warmup=20, latency_runs=100, batch_size_throughput=16, input_size=(256, 256), output_dir='results'):
    ...
    benchmark = run_full_benchmark(model_fn=lambda x: model(x), data_loader=test_loader, device=device, precision=precision, warmup=warmup, latency_runs=latency_runs, batch_size_throughput=batch_size_throughput, input_size=input_size)
    result = {'method': 'Baseline FP16' if precision == 'fp16' else 'Baseline FP32', 'precision': precision, 'device': device, 'runtime_device': device, 'model_size_mb': model_size_mb(model), 'p50_latency_ms': benchmark['latency']['median_ms'], 'p95_latency_ms': benchmark['latency']['p95_ms'], 'mean_latency_ms': benchmark['latency']['mean_ms'], 'throughput_img_sec': benchmark['throughput']['throughput_img_sec'], 'batch_size': batch_size_throughput, **quality, **{f'memory_{k}': v for (k, v) in benchmark['memory'].items()}, 'status': 'ok'}
