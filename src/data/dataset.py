...
def _limit_subset(dataset, max_samples=None, seed=42):
...
def build_dataloaders(data_root, image_size=256, batch_size=8, val_fraction=0.15, num_workers=0, seed=42, train_max_samples=None, val_max_samples=None):
    full_train = PetSegmentationDataset(
        root=data_root, split="trainval", image_size=image_size, augment=True
    )
    ...
    val_ds.dataset = PetSegmentationDataset(
        root=data_root, split="trainval", image_size=image_size, augment=False
    )

    train_ds = _limit_subset(train_ds, train_max_samples, seed=seed)
    val_ds = _limit_subset(val_ds, val_max_samples, seed=seed + 1)
...
def build_test_loader(data_root, image_size=256, batch_size=8, num_workers=0, max_samples=None, seed=42):
    test_ds = PetSegmentationDataset(root=data_root, split="test",
                                     image_size=image_size, augment=False)
    test_ds = _limit_subset(test_ds, max_samples, seed=seed)
