...
def _semi_structured_speedup_support(device):
    capability = _cuda_capability(device)
    if capability is None:
        return False, "2:4 sparse tensor cores require a CUDA GPU backend."

    major = int(capability.split("_")[1][0])
    if major < 8:
        return False, (
            f"{capability} does not support NVIDIA 2:4 sparse tensor core speedups "
            "(V100 is sm_70)."
        )

    return False, (
        "This PyTorch Conv2d path applies a dense mask only; it does not lower UNet "
        "convolutions to cuSPARSELt/TensorRT 2:4 sparse kernels."
    )
...
def run_unstructured_pruning_benchmark(model, train_loader, test_loader, device, output_dir="results", precision="fp16", prune_amount=0.5, finetune_epochs=2, finetune_lr=5e-5, finetune_weight_decay=1e-5, max_finetune_batches=None, warmup=20, latency_runs=100, batch_size_throughput=16, input_size=(256, 256)):
    logger = get_logger("prune_benchmark")
    logger.info(
        "Running unstructured pruning | amount=%.2f | finetune_epochs=%d",
        prune_amount,
        finetune_epochs,
    )
    ...
    benchmark = run_full_benchmark(
        model_fn=lambda x: infer_model(x),
    ...
    result = {
        ...
        "sparse_kernel_backend": None,
        "speedup_valid": False,
        "speedup_note": "Weights are zeroed in dense Conv2d tensors; no sparse kernels are used.",
        "model_size_mb": model_size_mb(infer_model),
        "effective_model_size_mb": model_size_mb(infer_model) * (1.0 - sparsity),
        "theoretical_nonzero_weight_size_mb": model_size_mb(infer_model) * (1.0 - sparsity),
...
def run_semi_structured_2_4_benchmark(model, train_loader, test_loader, device, output_dir="results", precision="fp16", finetune_epochs=2, finetune_lr=5e-5, finetune_weight_decay=1e-5, max_finetune_batches=None, warmup=20, latency_runs=100, batch_size_throughput=16, input_size=(256, 256)):
    ...
    sparsity = compute_weight_sparsity(model)
    sparse_speedup_supported, sparse_reason = _semi_structured_speedup_support(device)
    ...
    result = {
        "method": "2:4 Semi-structured",
        "precision": precision,
        "device": device,
        "runtime_device": device,
        "weight_sparsity": sparsity,
        "storage_format": "dense_mask",
        "sparse_kernel_backend": None,
        "sparse_speedup_supported": sparse_speedup_supported,
        "speedup_valid": sparse_speedup_supported,
        "speedup_note": sparse_reason,
        "cuda_capability": _cuda_capability(device),
        "model_size_mb": model_size_mb(infer_model),
        "effective_model_size_mb": model_size_mb(infer_model) * (1.0 - sparsity),
        "theoretical_nonzero_weight_size_mb": model_size_mb(infer_model) * (1.0 - sparsity),
        "p50_latency_ms": benchmark["latency"]["median_ms"],
