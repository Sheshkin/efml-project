...
def run_compile_benchmark(model, test_loader, device, precision="fp16", compile_mode="default",
                          warmup=20, latency_runs=100, batch_size_throughput=16,
    ...
    benchmark = run_full_benchmark(
        model_fn=lambda x: compiled_model(x),
    ...
    result = {
        "method": f"torch.compile ({safe_mode})",
        "precision": precision,
        "device": device,
