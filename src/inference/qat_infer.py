...
class _QATTrainer:
    def __init__(self, model, device, lr=1e-5, weight_decay=1e-5):
...
def _serialized_model_size_mb(model):
    ...
    try:
        ...
        return os.path.getsize(temp_path) / 1e6
    finally:
...
def run_qat_benchmark(model, train_loader, test_loader, output_dir="results", train_device="cpu", qat_epochs=2, qat_lr=1e-5, qat_weight_decay=1e-5, max_qat_batches=None, warmup=20, latency_runs=100, batch_size_throughput=16, input_size=(256, 256)):
    ...
    benchmark = run_full_benchmark(
        model_fn=lambda x: int8_model(x),
    ...
    result = {
        ...
        "runtime_device": "cpu",
        "quant_engine": engine,
        "model_size_mb": _serialized_model_size_mb(int8_model),
