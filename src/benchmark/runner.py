...
def measure_throughput(fn, input_tensor, batch_size, device, warmup=5, runs=20):
    ...
    elapsed = t1 - t0
    return {
        "throughput_img_sec": (batch_size * runs) / elapsed,
...
def measure_memory(device):
    ...
    if device == "cuda":
        ...
        result["peak_mb"] = torch.cuda.max_memory_allocated() / 1e6
    elif device == "mps":
        result["allocated_mb"] = torch.mps.current_allocated_memory() / 1e6
        result["peak_mb"] = result["allocated_mb"]
    else:
        import psutil
        proc = psutil.Process(os.getpid()) if _psutil_available() else None
        result["rss_mb"] = proc.memory_info().rss / 1e6 if proc else 0.0
    return result
...
def run_full_benchmark(model_fn, data_loader, device, precision="fp32", warmup=20,
                       latency_runs=100, batch_size_throughput=16, input_size=(256, 256)):
    C = 3
    H, W = input_size
